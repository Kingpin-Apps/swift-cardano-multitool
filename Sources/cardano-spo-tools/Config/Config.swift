...
@dynamicMemberLookup
struct CardanoSPOToolsConfigs: Codable, Sendable {
    ...
    /// Creates a new CardanoSPOToolsConfigs using values from the provided reader.
    ///
    ...
    func save(to path: FilePath, overwrite: Bool = false) throws {
        if FileManager.default.fileExists(atPath: path.string) && !overwrite {
            throw CardanoSPOToolsError.fileAlreadyExists("File already exists: \(path)")
        }
    ...
    static func load() async throws -> CardanoSPOToolsConfigs {
    ...
    /// - Throws: An error if the file cannot be read or parsed.
    /// - Note: Environment variables will override values in the JSON file.
    static func load(from path: FilePath) async throws -> CardanoSPOToolsConfigs {
        
        let config = ConfigReader(providers: [
            EnvironmentVariablesProvider(),
            try await JSONProvider(filePath: .init(path.string))
        ])
        return CardanoSPOToolsConfigs(config: config)
    }
}
...
/// Main configuration structure for Cardano SPO tools
public struct CardanoSPOToolsConfig: Codable, Sendable {
    ...
    static func `default`() throws -> CardanoSPOToolsConfig {
        let cwd = FilePath(FileManager.default.currentDirectoryPath)
        return CardanoSPOToolsConfig(
    ...
    /// - Throws: An error if the file cannot be read or parsed.
    /// - Note: Environment variables will override values in the JSON file.
    static func load() async throws -> CardanoSPOToolsConfig {
    ...
    /// - Throws: An error if the file cannot be read or parsed.
    /// - Note: Environment variables will override values in the JSON file.
    static func load(from path: FilePath) async throws -> CardanoSPOToolsConfig {
        
        let config = ConfigReader(providers: [
            EnvironmentVariablesProvider(),
            try await JSONProvider(filePath: .init(path.string))
        ])
        return CardanoSPOToolsConfig(config: config)
    }
    
    func toCLIToolsConfig() -> CardanoCLIToolsConfig {

...
extension SignMainCommand {
    ...
    struct SignCIP8: AsyncParsableCommand {
        static let configuration = CommandConfiguration(
            commandName: "cip8",
            abstract: "Sign a payload as a CIP-8 COSE_Sign1 message bound to an address.",
            usage: """
        ...
        @Flag(name: .customLong("attach-cose-key"), help: "Attach the verification key as a separate COSE_Key (CIP-30 shape).")
        var attachCoseKey: Bool = false

        @OptionGroup var output: SignerOutputOptions

        mutating func wizard() async throws {
            let source = SignerUtils.promptDataSource(title: "CIP-8 data to sign")
            switch source {
            case .text:
            ...
            attachCoseKey = noora.yesOrNoChoicePrompt(
                title: "Attach COSE Key",
                question: "Attach the verification key as a separate COSE_Key?",
                defaultAnswer: false,
                description: "Required for CIP-30 dApp signData responses."
            )
        }
        ...
        mutating func run() async throws {
            ...
            let signed = try Signer.CIP8.sign(
                payload: payload,
                signingKey: key,

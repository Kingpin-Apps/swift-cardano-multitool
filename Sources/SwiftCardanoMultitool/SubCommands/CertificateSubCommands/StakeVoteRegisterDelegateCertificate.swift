...
extension CertificateMainCommand {
    ...
    struct StakeVoteRegisterDelegateCertificate: CertificateCommandable {
        static let configuration = CommandConfiguration(
            commandName: "stake-vote-register-delegation",
            abstract: "Generates a stake address registration, stake delegation and vote delegation certificate.",
            usage: """
            scm certificate stake-vote-register-delegation --stake-address owner --pool-operator pool1xyz --drep drep1abc
            """,
            discussion: """
            Creates a combined stake address registration, stake delegation and
            vote delegation certificate. This Conway-era certificate registers
            the stake address on-chain, delegates its stake to the specified pool,
            and delegates its voting power to the specified DRep — all in a single
        ...
        mutating func run() async throws {
            if stakeAddress == nil || poolOperator == nil || drep == nil , isInteractiveSession() {
                try await wizard()
            }
            ...
            if certificateOptions.outFile == nil {
                certificateOptions.outFile = cwd.appending("\(stakeVkeyFilePath.stem!)-\(timestamp).stake-vote-reg-deleg.cert")
            }
            ...
            print(noora.format("\nGenerating stake registration, delegation and vote delegation certificate for: \(.primary(stakeAddress.info.name!))"))
            ...
            do {
                if transactionOptions.useCardanoCLI {
                    ...
                    _ = try await cli.stakeAddress.registrationStakeAndVoteDelegationCertificate(arguments: arguments)
                    ...
                    let cert = SwiftCardanoCore.StakeVoteRegisterDelegate(
            ...
            noora.success(.alert(
                "Stake Registration, Delegation and Vote Delegation certificate created successfully.",
                takeaways: [
                    "File: \(outFile.string)",
                    "Registers stake address, delegates to pool: \(.primary(try poolOperator.id()))",
            ...
            if certificateOptions.generateTransaction {
                ...
                let loadedCert = try SwiftCardanoCore.StakeVoteRegisterDelegate.load(from: outFile.string)
                txBuilder.certificates = [.stakeVoteRegisterDelegate(loadedCert)]

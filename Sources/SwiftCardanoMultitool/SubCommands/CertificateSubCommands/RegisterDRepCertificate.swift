...
extension CertificateMainCommand {
    ...
    struct RegisterDRepCertificate: CertificateCommandable {
        static let configuration = CommandConfiguration(
            commandName: "register-drep",
            abstract: "Generates a DRep registration certificate.",
            usage: """
            scm certificate register-drep --drep-credential drep1abc
            """,
            discussion: """
            Creates a DRep registration certificate that registers a Delegation
            Representative on the Cardano blockchain. The DRep deposit is
            required and deducted from the fee payment address. An optional
            anchor (URL + metadata hash) can be provided to link the
            registration to off-chain metadata (CIP-100). If the
        ...
        mutating func wizard() async throws {
            drepCredential = try await getDRepCredential(title: "DRep Credential to Register")
        ...
        mutating func run() async throws {
            ...
            let anchor = try await getOptionalAnchor(purpose: "DRep registration")
            ...
            if certificateOptions.outFile == nil {
                certificateOptions.outFile = cwd.appending("\(drepIdHex.prefix(8))-\(timestamp).drep-reg.cert")
            }
            ...
            print(noora.format("\nGenerating DRep registration certificate"))
            print(noora.format("  DRep: \(.primary(try drepCredential.id()))"))
            spacedPrint("DRep Deposit: \(.primary("\(lovelaceToAdaFormatString(UInt64(drepDeposit)))")) / \(drepDeposit) lovelaces.")
            ...
            do {
                if transactionOptions.useCardanoCLI {
                    ...
                    arguments.append(contentsOf: ["--key-reg-deposit-amt", "\(drepDeposit)"])
                    if let anchor = anchor {
                    ...
                    _ = try await cli.governance.drepRegistration(arguments: arguments)
                    try await FileUtils.fileLock(outFile)
                } else {
                    let cert = SwiftCardanoCore.RegisterDRep(
            ...
            noora.success(.alert(
                "DRep Registration certificate created successfully.",
                takeaways: [
                    "File: \(outFile.string)",
                    "DRep: \(.primary(try drepCredential.id()))",
                    "Deposit: \(drepDeposit) lovelaces",
            ...
            if certificateOptions.generateTransaction {
                ...
                let loadedCert = try SwiftCardanoCore.RegisterDRep.load(from: outFile.string)
                txBuilder.certificates = [.registerDRep(loadedCert)]

...
extension CertificateMainCommand {
    ...
    struct AuthCommitteeHotCertificate: CertificateCommandable {
        static let configuration = CommandConfiguration(
            commandName: "auth-committee-hot",
            abstract: "Generates a constitutional committee hot key authorization certificate.",
            usage: """
            scm certificate auth-committee-hot --committee-cold-credential cc_cold1... --committee-hot-credential cc_hot1...
            """,
            discussion: """
            Creates a constitutional committee hot key authorization certificate
            that authorizes a hot key to act on behalf of a cold key. The cold
            credential identifies the committee member and the hot credential is
            the key that will be used for day-to-day governance actions. If the
        ...
        mutating func wizard() async throws {
            committeeColdCredential = try await getCommitteeColdCredential(title: "Committee Cold Credential")
            committeeHotCredential = try await getCommitteeHotCredential(title: "Committee Hot Credential to Authorize")
        ...
        mutating func run() async throws {
            if committeeColdCredential == nil || committeeHotCredential == nil , isInteractiveSession() {
                try await wizard()
            }
            ...
            guard let hotCredential = committeeHotCredential else {
                noora.error(.alert("Committee hot credential is required.", takeaways: ["Provide a valid cc_hot credential."]))
                throw ExitCode.validationFailure
            }
            ...
            if certificateOptions.outFile == nil {
                certificateOptions.outFile = cwd.appending("\(coldIdHex.prefix(8))-\(timestamp).committee-hot-auth.cert")
            }
            ...
            print(noora.format("\nGenerating committee hot key authorization certificate"))
            print(noora.format("  Cold: \(.primary(try coldCredential.id()))"))
            print(noora.format("  Hot:  \(.primary(try hotCredential.id()))"))
            ...
            do {
                if transactionOptions.useCardanoCLI {
                    ...
                    switch hotCredential.credential {
                        case .verificationKeyHash(let hash):
                            arguments.append(contentsOf: ["--hot-key-hash", hash.payload.toHex])
                        case .scriptHash(let hash):
                    ...
                    let cert = SwiftCardanoCore.AuthCommitteeHot(
                        committeeColdCredential: coldCredential,
                        committeeHotCredential: hotCredential
                    )
            ...
            noora.success(.alert(
                "Committee Hot Key Authorization certificate created successfully.",
                takeaways: [
                    "File: \(outFile.string)",
                    "Cold credential: \(.primary(try coldCredential.id()))",
                    "Hot credential: \(.primary(try hotCredential.id()))",
            ...
            if certificateOptions.generateTransaction {
                ...
                let loadedCert = try SwiftCardanoCore.AuthCommitteeHot.load(from: outFile.string)
                txBuilder.certificates = [.authCommitteeHot(loadedCert)]

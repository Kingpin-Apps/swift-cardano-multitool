...
extension InstallMainCommand {
    struct CardanoWallet: AsyncParsableCommand {
        static let configuration = CommandConfiguration(
            commandName: "cardano-wallet",
            abstract: "Install cardano-wallet."
        )
        ...
        private static let officialImage = "cardanofoundation/cardano-wallet"

        mutating func wizard() async throws {
            if method == nil {
                let selected: InstallMethod = noora.singleChoicePrompt(
                    title: "Install Method",
                    question: "How would you like to install cardano-wallet?",
        ...
        private func installBinaryRelease() async throws {
            ...
            let release = try await noora.progressStep(
                message: "Fetching latest cardano-wallet release...",
                successMessage: "Found latest release.",
                errorMessage: "Failed to fetch release information.",
                showSpinner: true
            ) { _ in
                return try await fetchLatestRelease(owner: "cardano-foundation", repo: "cardano-wallet")
            }
            ...
            guard let asset = findMatchingAsset(in: release) else {
                noora.error(.alert(
                    ...
                    takeaways: [
                        "Check available assets at: https://github.com/cardano-foundation/cardano-wallet/releases",
            ...
            let _ = try await noora.progressStep(
                message: "Downloading and installing cardano-wallet \(release.tagName)...",
                successMessage: "cardano-wallet \(release.tagName) installed to \(installDirURL.path)",
                errorMessage: "Failed to install cardano-wallet.",
                showSpinner: true
            ) { _ in
                ...
                try processDownloadedAsset(archivePath: archivePath, binaryName: "cardano-wallet", installDir: installDirURL)

...
extension InstallMainCommand {
    struct Mithril: AsyncParsableCommand {
        static let configuration = CommandConfiguration(
            commandName: "mithril",
            abstract: "Install mithril-client."
        )
        ...
        mutating func wizard() async throws {
            if method == nil {
                let selected: InstallMethod = noora.singleChoicePrompt(
                    ...
                    question: "How would you like to install mithril-client?",
            ...
            if isContainerMethod && image == nil {
                let defaultImage = "\(OfficialImage.mithril.rawValue):latest"
                let useDefault = noora.yesOrNoChoicePrompt(
        ...
        mutating func run() async throws {
            ...
            switch installMethod {
                ...
                case .docker:
                    let img = image ?? "\(OfficialImage.mithril.rawValue):latest"
        ...
        private func installBinaryRelease() async throws {
            ...
            let release = try await noora.progressStep(
                message: "Fetching latest mithril release...",
                successMessage: "Found latest release.",
                errorMessage: "Failed to fetch release information.",
                showSpinner: true
            ) { _ in
                return try await fetchLatestRelease(owner: "input-output-hk", repo: "mithril")
            }

            // Mithril releases include multiple binaries; filter for the client archive.
            guard let asset = findMatchingAsset(
                in: release,
                excludeKeywords: ["sha256", "checksum", ".sig", ".asc", "source", "-src", "-debug",
                                  "windows", "-win", "aggregator", "-signer", "relay"]
            ) else {
                noora.error(.alert(
                    "No compatible mithril-client binary found for your platform (\(CurrentPlatform.os)/\(CurrentPlatform.arch)).",
                    takeaways: [
                        "Check available assets at: https://github.com/input-output-hk/mithril/releases",
            ...
            let _ = try await noora.progressStep(
                message: "Downloading and installing mithril-client \(release.tagName)...",
                successMessage: "mithril-client \(release.tagName) installed to \(installDirURL.path)",
                errorMessage: "Failed to install mithril-client.",
                showSpinner: true
            ) { _ in
                ...
                try processDownloadedAsset(archivePath: archivePath, binaryName: "mithril-client", installDir: installDirURL)

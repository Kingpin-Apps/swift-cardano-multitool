    ...
    struct Ogmios: AsyncParsableCommand {
        ...
        mutating func wizard() async throws {
            if method == nil {
                let selected: InstallMethod = noora.singleChoicePrompt(
                    ...
                    question: "How would you like to install Ogmios?",
            ...
            if isContainerMethod && image == nil {
                let defaultImage = "\(OfficialImage.ogmios.rawValue):latest"
                let useDefault = noora.yesOrNoChoicePrompt(
        ...
        mutating func run() async throws {
            ...
            switch installMethod {
                ...
                case .docker:
                    let img = image ?? "\(OfficialImage.ogmios.rawValue):latest"
        ...
        private func installBinaryRelease() async throws {
            ...
            let release = try await noora.progressStep(
                message: "Fetching latest Ogmios release...",
                successMessage: "Found latest release.",
                errorMessage: "Failed to fetch release information.",
                showSpinner: true
            ) { _ in
                return try await fetchLatestRelease(owner: "CardanoSolutions", repo: "ogmios")
            }
            ...
            guard let asset = findMatchingAsset(in: release) else {
                noora.error(.alert(
                    ...
                    takeaways: [
                        "Check available assets at: https://github.com/CardanoSolutions/ogmios/releases",
            ...
            let _ = try await noora.progressStep(
                message: "Downloading and installing Ogmios \(release.tagName)...",
                successMessage: "Ogmios \(release.tagName) installed to \(installDirURL.path)",
                errorMessage: "Failed to install Ogmios.",
                showSpinner: true
            ) { _ in
                ...
                try processDownloadedAsset(archivePath: archivePath, binaryName: "ogmios", installDir: installDirURL)

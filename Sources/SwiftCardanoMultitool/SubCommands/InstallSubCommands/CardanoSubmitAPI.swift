...
extension InstallMainCommand {
    struct CardanoSubmitAPI: AsyncParsableCommand {
        static let configuration = CommandConfiguration(
            commandName: "cardano-submit-api",
            abstract: "Install cardano-submit-api."
        )
        ...
        @Option(
            ...
            help: "Container image to pull. If omitted, the cardano-node image is used (it bundles cardano-submit-api)."
        )
        var image: String?
        ...
        mutating func wizard() async throws {
            if method == nil {
                let selected: InstallMethod = noora.singleChoicePrompt(
                    ...
                    question: "How would you like to install cardano-submit-api?",
            ...
            if isContainerMethod && image == nil {
                ...
                let useDefault = noora.yesOrNoChoicePrompt(
                    ...
                    question: "Use cardano-node image (\(defaultImage))? (No standalone cardano-submit-api image exists.)",
                    defaultAnswer: true,
        ...
        mutating func run() async throws {
            ...
            switch installMethod {
                ...
                case .docker:
                    ...
                    if image == nil {
                        noora.warning(.alert(
                            "No standalone cardano-submit-api Docker image is available.",
                            takeaway: "Pulling '\(OfficialImage.cardanoNode.rawValue)' instead, which bundles cardano-submit-api."
                        ))
                    }
                ...
                case .appleContainer:
                    ...
                    if image == nil {
                        noora.warning(.alert(
                            "No standalone cardano-submit-api Apple Container image is available.",
                            takeaway: "Pulling '\(OfficialImage.cardanoNode.rawValue)' instead, which bundles cardano-submit-api."
                        ))
                    }
        ...
        private func installBinaryRelease() async throws {
            ...
            let release = try await noora.progressStep(
                message: "Fetching latest cardano-node release (bundles cardano-submit-api)...",
            ...
            let _ = try await noora.progressStep(
                message: "Downloading and installing cardano-submit-api \(release.tagName)...",
                successMessage: "cardano-submit-api \(release.tagName) installed to \(installDirURL.path)",
                errorMessage: "Failed to install cardano-submit-api.",
                showSpinner: true
            ) { _ in
                ...
                try processDownloadedAsset(archivePath: archivePath, binaryName: "cardano-submit-api", installDir: installDirURL)

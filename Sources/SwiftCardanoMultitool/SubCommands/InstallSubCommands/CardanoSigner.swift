...
extension InstallMainCommand {
    struct CardanoSigner: AsyncParsableCommand {
        static let configuration = CommandConfiguration(
            commandName: "cardano-signer",
            abstract: "Install cardano-signer."
        )
        ...
        mutating func wizard() async throws {
            if method == nil {
                let selected: InstallMethod = noora.singleChoicePrompt(
                    ...
                    question: "How would you like to install cardano-signer?",
            ...
            if isContainerMethod && image == nil {
                image = noora.textPrompt(
                    ...
                    prompt: "Enter the full image name (e.g. myregistry/cardano-signer:latest):"
                )
            }
        }
            ...
            switch installMethod {
                ...
                case .docker:
                    guard let img = image, !img.isEmpty else {
                        noora.error(.alert(
                            "No container image specified for cardano-signer.",
                            takeaways: ["Use --image to specify a Docker image (e.g. --image myregistry/cardano-signer:latest)."]
                        ))
                        throw ExitCode.failure
                    }
                ...
                case .appleContainer:
                    guard let img = image, !img.isEmpty else {
                        noora.error(.alert(
                            "No container image specified for cardano-signer.",
                            takeaways: ["Use --image to specify an Apple Container image (e.g. --image myregistry/cardano-signer:latest)."]
                        ))
                        throw ExitCode.failure
                    }
        ...
        private func installBinaryRelease() async throws {
            ...
            let release = try await noora.progressStep(
                message: "Fetching latest cardano-signer release...",
                successMessage: "Found latest release.",
                errorMessage: "Failed to fetch release information.",
                showSpinner: true
            ) { _ in
                return try await fetchLatestRelease(owner: "gitmachtl", repo: "cardano-signer")
            }

            guard let asset = findMatchingAsset(
                in: release,
                excludeKeywords: ["sha256", "checksum", ".sig", ".asc", "source", "-src", "-debug", "windows", "-win", ".js", ".pkg"]
            ) else {
                noora.error(.alert(
                    "No compatible binary found for your platform (\(CurrentPlatform.os)/\(CurrentPlatform.arch)).",
                    takeaways: [
                        "Check available assets at: https://github.com/gitmachtl/cardano-signer/releases",
            ...
            let _ = try await noora.progressStep(
                message: "Downloading and installing cardano-signer \(release.tagName)...",
                successMessage: "cardano-signer \(release.tagName) installed to \(installDirURL.path)",
                errorMessage: "Failed to install cardano-signer.",
                showSpinner: true
            ) { _ in
                ...
                try processDownloadedAsset(archivePath: archivePath, binaryName: "cardano-signer", installDir: installDirURL)
